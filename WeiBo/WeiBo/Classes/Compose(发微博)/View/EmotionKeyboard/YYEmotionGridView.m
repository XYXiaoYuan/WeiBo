...
@interface YYEmotionGridView ()
@property (nonatomic, weak) UIButton *deleteButton;
@property (nonatomic, strong) NSMutableArray *emotionViews;
@property (nonatomic, strong) YYEmotionPopView *popView;
@end

@implementation YYEmotionGridView

- (YYEmotionPopView *)popView
{
    if (!_popView) {
        self.popView = [YYEmotionPopView popView];
    }
    return _popView;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [deleteButton setImage:[UIImage yy_imageWithName:@"compose_emotion_delete"] forState:UIControlStateNormal];
        [deleteButton setImage:[UIImage yy_imageWithName:@"compose_emotion_delete_highlighted"] forState:UIControlStateHighlighted];
 ...
 *  根据触摸点返回对应的表情控件
 */
- (YYEmotionView *)emotionViewWithPoint:(CGPoint)point
{
    __block YYEmotionView *foundEmotionView = nil;
    [self.emotionViews enumerateObjectsUsingBlock:^(YYEmotionView *emotionView, NSUInteger idx, BOOL *stop) {
...
- (void)longPress:(UILongPressGestureRecognizer *)recognizer
{
    ...
    // 2.检测触摸点落在哪个表情上
    YYEmotionView *emotionView = [self emotionViewWithPoint:point];
...
- (void)setEmotions:(NSArray *)emotions
{
    ...
    for (int i = 0; i<count; i++) {
        YYEmotionView *emotionView = nil;
        
        if (i >= currentEmotionViewCount) { // emotionView不够用
            emotionView = [[YYEmotionView alloc] init];
            //            emotionView.backgroundColor = YYRandomColor;
 ...
 *  监听表情的单击
 */
- (void)emotionClick:(YYEmotionView *)emotionView
{
    [self.popView showFromEmotionView:emotionView];
 ...
 *  选中表情
 */
- (void)selecteEmotion:(YYEmotion *)emotion
{
    if (emotion == nil) return;
#warning 注意：先添加使用的表情，再发通知
    // 保存使用记录
    [YYEmotionTool addRecentEmotion:emotion];
    
    // 发出一个选中表情的通知
    [[NSNotificationCenter defaultCenter] postNotificationName:YYEmotionDidSelectedNotification object:nil userInfo:@{YYSelectedEmotion : emotion}];
}
...
{
    // 发出一个选中表情的通知
    [[NSNotificationCenter defaultCenter] postNotificationName:YYEmotionDidDeletedNotification object:nil userInfo:nil];
}
...
- (void)layoutSubviews
{
    ...
    CGFloat emotionViewW = (self.width - 2 * leftInset) /YYEmotionMaxCols;
    CGFloat emotionViewH = (self.height - topInset) / YYEmotionMaxRows;
    for (int i = 0; i<count; i++) {
        UIButton *emotionView = self.emotionViews[i];
        emotionView.x = leftInset + (i % YYEmotionMaxCols) * emotionViewW;
        emotionView.y = topInset + (i / YYEmotionMaxCols) * emotionViewH;
