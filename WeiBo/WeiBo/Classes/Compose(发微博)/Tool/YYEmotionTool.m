...
#define YYRecentFilepath [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"recent_emotions.data"]

#import "YYEmotionTool.h"
#import "YYEmotion.h"
#import "MJExtension.h"

@implementation YYEmotionTool
...
+ (NSArray *)defaultEmotions
{
    if (!_defaultEmotions) {
        ...
        _defaultEmotions = [YYEmotion mj_objectArrayWithFile:plist];
...
+ (NSArray *)emojiEmotions
{
    if (!_emojiEmotions) {
        ...
        _emojiEmotions = [YYEmotion mj_objectArrayWithFile:plist];
...
+ (NSArray *)lxhEmotions
{
    if (!_lxhEmotions) {
        ...
        _lxhEmotions = [YYEmotion mj_objectArrayWithFile:plist];
...
+ (NSArray *)recentEmotions
{
    if (!_recentEmotions) {
        // 去沙盒中加载最近使用的表情数据
        _recentEmotions = [NSKeyedUnarchiver unarchiveObjectWithFile:YYRecentFilepath];
...
+ (void)addRecentEmotion:(YYEmotion *)emotion
{
    ...
    // 存储到沙盒中
    [NSKeyedArchiver archiveRootObject:_recentEmotions toFile:YYRecentFilepath];
}
...
+ (YYEmotion *)emotionWithDesc:(NSString *)desc
{
    ...
    __block YYEmotion *foundEmotion = nil;
    
    // 从默认表情中找
    [[self defaultEmotions] enumerateObjectsUsingBlock:^(YYEmotion *emotion, NSUInteger idx, BOOL *stop) {
    ...
    // 从浪小花表情中查找
    [[self lxhEmotions] enumerateObjectsUsingBlock:^(YYEmotion *emotion, NSUInteger idx, BOOL *stop) {
        if ([desc isEqualToString:emotion.chs] || [desc isEqualToString:emotion.cht]) {
            foundEmotion = emotion;
            *stop = YES;
        }
