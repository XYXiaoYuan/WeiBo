...
@property (nonatomic, weak) YYComposeToolbar *toolbar;
@property (nonatomic, weak) YYComposePhotosView *photosView;
@property (nonatomic, strong) YYEmotionKeyboard *kerboard;
/**
 *  是否正在切换键盘
 */
...
#pragma mark - 初始化方法
- (YYEmotionKeyboard *)kerboard
{
    if (!_kerboard) {
        self.kerboard = [YYEmotionKeyboard keyboard];
        self.kerboard.width = YYScreenW;
...
- (void)viewDidLoad
{
    ...
    // 监听删除按钮点击的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(emotionDidDeleted:) name:YYEmotionDidDeletedNotification object:nil];
}

// 添加显示图片的相册控件
- (void)setupPhotosView
{
    YYComposePhotosView *photosView = [[YYComposePhotosView alloc] init];
...
{
    // 1.创建
    YYComposeToolbar *toolbar = [[YYComposeToolbar alloc] init];
...
{
    // 1.创建输入控件
    YYEmotionTextView *textView = [[YYEmotionTextView alloc] init];
...
{
    // 1.封装请求参数
    YYSendStatusParam *param = [YYSendStatusParam param];
    param.status = self.textView.realText;
    
    // 2.发微博
    [YYStatusTool sendStatusWithParam:param success:^(YYSendStatusResult *result) {
        [MBProgressHUD showSuccess:@"发表成功"];
...
#pragma mark - YYComposeToolbarDelegate
/**
 *  监听toolbar内部按钮的点击
 */
...
{
    switch (buttonType) {
        case YYComposeToolbarButtonTypeCamera: // 照相机
            [self openCamera];
            break;
            
        case YYComposeToolbarButtonTypePicture: // 相册
            [self openAlbum];
            break;
            
        case YYComposeToolbarButtonTypeEmotion: // 表情
            [self openEmotion];
            break;
...
{
    YYEmotion *emotion = note.userInfo[YYSelectedEmotion];
