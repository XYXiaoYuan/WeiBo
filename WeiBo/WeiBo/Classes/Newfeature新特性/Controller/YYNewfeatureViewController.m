...
#define YYNewfeatureImageCount 4

@interface YYNewfeatureViewController () <UIScrollViewDelegate>
@property (nonatomic, weak) UIPageControl *pageControl;

@end

@implementation YYNewfeatureViewController
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < YYNewfeatureImageCount; i++) {
        // 创建UIImageView
        ...
        // 给最后一个imageView添加按钮
        if (i == YYNewfeatureImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置其他属性
    scrollView.contentSize = CGSizeMake(YYNewfeatureImageCount * imageW, 0);
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = YYNewfeatureImageCount;
    pageControl.centerX = self.view.width * 0.5;
    pageControl.centerY = self.view.height - 30;
    [self.view addSubview:pageControl];
    
    // 2.设置圆点的颜色
    pageControl.currentPageIndicatorTintColor = YYColor(253, 98, 42); // 当前页的小圆点颜色
    pageControl.pageIndicatorTintColor = YYColor(189, 189, 189); // 非当前页的小圆点颜色
...
- (void)setupShareButton:(UIImageView *)imageView
{
    ...
    [shareButton setImage:[UIImage yy_imageWithName:@"new_feature_share_false"] forState:UIControlStateNormal];
    [shareButton setImage:[UIImage yy_imageWithName:@"new_feature_share_true"] forState:UIControlStateSelected];
    // 监听点击
...
- (void)setupStartButton:(UIImageView *)imageView
{
    ...
    // 2.设置背景图片
    [startButton setBackgroundImage:[UIImage yy_imageWithName:@"new_feature_finish_button"] forState:UIControlStateNormal];
    [startButton setBackgroundImage:[UIImage yy_imageWithName:@"new_feature_finish_button_highlighted"] forState:UIControlStateHighlighted];
...
- (void)start
{
    ...
    // 显示主控制器（YYTabBarController）
    YYTabBarViewController *vc = [[YYTabBarViewController alloc] init];
...
- (void)dealloc
{
//    YYLog(@"dealloc-------YYNewfeatureViewController");
}
