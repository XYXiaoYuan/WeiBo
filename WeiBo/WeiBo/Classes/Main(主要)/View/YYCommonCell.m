...
#import "YYCommonCenterItem.h"
#import "YYBadgeView.h"

@interface YYCommonCell ()
/**  箭头 */
...
/**  提醒数字 */
@property (strong, nonatomic) YYBadgeView *bageView;
...
{
    if (_rightArrow == nil) {
        self.rightArrow = [[UIImageView alloc] initWithImage:[UIImage yy_imageWithName:@"common_icon_arrow"]];
    }
    return _rightArrow;
}
...
- (YYBadgeView *)bageView
{
    if (_bageView == nil) {
        self.bageView = [[YYBadgeView alloc] init];
    }
    return _bageView;
}
...
- (void)setIndexPath:(NSIndexPath *)indexPath rowsInSection:(int)rows
{
    ...
    // 2.设置背景图片
    if (rows == 1) {
        bgView.image = [UIImage yy_resizedImage:@"common_card_background"];
        selectedBgView.image = [UIImage yy_resizedImage:@"common_card_background_highlighted"];
    } else if (indexPath.row == 0) { // 首行
        bgView.image = [UIImage yy_resizedImage:@"common_card_top_background"];
        selectedBgView.image = [UIImage yy_resizedImage:@"common_card_top_background_highlighted"];
    } else if (indexPath.row == rows - 1) { // 末行
        bgView.image = [UIImage yy_resizedImage:@"common_card_bottom_background"];
        selectedBgView.image = [UIImage yy_resizedImage:@"common_card_bottom_background_highlighted"];
    } else { // 中间
        bgView.image = [UIImage yy_resizedImage:@"common_card_middle_background"];
        selectedBgView.image = [UIImage yy_resizedImage:@"common_card_middle_background_highlighted"];
    }
}
...
- (void)setItem:(YYCommonItem *)item
{
    ...
    // 1.设置基本数据
    self.imageView.image = [UIImage yy_imageWithName:item.icon];
    ...
    } else if ([item isKindOfClass:[YYCommonLabelItem class]]) {
        YYCommonLabelItem *labelItem = (YYCommonLabelItem *)item;
        // 设置文字
